...
export async function POST(request: Request) {
  try {
    ...
    // Resend API or Custom SMTP integration
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
          ...
          <div style="padding: 24px; background-color: #ffffff;">
            <p style="font-size: 14px; color: #334155; margin-top: 0;">
              Hi <strong>${agentName}</strong>, a new property buyer has just been assigned to you. Please make the initial outreach call as soon as possible.
            </p>
    ...
    return NextResponse.json({ 
      success: true, 
      message: 'Notification logged (Resend API key not configured yet)' 
    });
  } catch (err: any) {
